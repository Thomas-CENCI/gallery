...
def get_images(path):
    ...
    result[randrange(len(result))]["title_thumbnail"] = True
    return result
...
def run():
    ...
    for i, path in enumerate(dirs):
        print(f"{i+1}: Processing photos for album '{path}'")
        config[path] = get_images(path)
        print(f"\tDone processing {len(config[path])} photos for '{path}'\n")

    print(f"Done processing all {len(dirs)} albums")
    print(f"Writing files to {PATH}config.json")
    write_config(config)
    print("""Writing done!""")
    return 0
